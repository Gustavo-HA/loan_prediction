...
def test_prepare_features():
    ...
    preprocessor = joblib.load(open("./models/preprocessor.pkl", "rb"))

    actual_features_str = preprocessor.transform(pd.DataFrame([input_record["input"]])).to_json(
        orient="records"
    )[1:-1]
...
def test_base64_decode():
    data = (
        "ewogICAgImlucHV0IiA6IHsKICAgICAgICAiR2VuZGVyIjogIk1hbGUiLAogICAgICAgICJNYXJyaWVkIjogIlllcy"
...
def test_lambda_handler():
    ...
    preprocessor = joblib.load(open("./models/preprocessor.pkl", "rb"))
    model_service = model_module.ModelService(
        model=model_mock, preprocessor=preprocessor, model_version=model_version
    )
